package com.sulake.habbo.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      private var var_514:BitmapData;
      
      private var var_1304:XML;
      
      private var var_315:IWindowContainer;
      
      private var var_440:IItemGridWindow;
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = null;
         var_440.destroyGridItems();
         var_315 = null;
         for each(_loc2_ in colors)
         {
            _loc3_ = param1.buildFromXML(var_1304) as IWindowContainer;
            _loc3_.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,onClick);
            _loc3_.background = true;
            _loc3_.color = 4294967295;
            _loc3_.width = var_514.width;
            _loc3_.height = var_514.height;
            var_440.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(var_514.width,var_514.height,true,0);
               _loc4_.bitmap.copyPixels(var_514,var_514.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(var_894.width,var_894.height,true,0);
      ...
      {
         var _loc2_:int = var_440.getGridItemIndex(param1.target as IWindow);
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= var_440.numGridItems)
         {
            return;
         }
         select(var_440.getGridItemAt(param1) as IWindowContainer);
      }
