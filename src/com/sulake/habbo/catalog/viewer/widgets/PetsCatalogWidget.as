package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_387 >= var_295.length)
         {
            return "";
         }
         var petColor:IPetColor = var_295[var_387] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_554[var_1408] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_387 = param1.index;
         if(var_387 < 0 || var_387 > var_295.length)
         {
            var_387 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_572.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         var_779 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_295 = [];
         var_554 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_779);
               if(_loc11_)
               {
                  var_554 = _loc11_.breeds;
                  var_295 = _loc11_.colors;
                  if(var_295 == null)
                  {
                     var_295 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_554.length)
         {
            _loc12_ = var_554[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_779,_loc12_.id),getRaceLocalizationKey(var_779,_loc12_.id)));
            _loc6_++;
         }
         var_572.add(var_779,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_911,onApproveNameResult);
         _loc5_ = var_572.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_295)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_572.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_387 >= 0 && var_387 < var_295.length)
         {
            _loc7_ = var_295[var_387] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_554[var_1408] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_779,_loc3_.id,_loc2_,AvatarScaleType.const_56);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_313,AvatarAction.const_541);
