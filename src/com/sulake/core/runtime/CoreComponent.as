package com.sulake.core.runtime
{
   ...
   public final class CoreComponent extends ComponentContext implements ICore
   {
      ...
      private static const const_1120:String = "zip-libraries";
      
      private static const const_1115:String = "library";
      
      private static const const_1117:String = "library";
      
      private static const const_1119:String = "asset-libraries";
      ...
      private var var_283:uint;
      
      private var var_282:Array;
      
      private var var_326:uint;
      
      private var var_1085:IEventDispatcher;
      
      private var var_924:Profiler;
      ...
      public function CoreComponent(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         var_1400 = (param2 & 0) == Core.CORE_SETUP_DEBUG;
         var_533 = new Array();
         var_627 = new Array();
         var_215 = param1;
         var _loc3_:int = 0;
         ...
         switch(param2 & 0)
         {
            ...
            case Core.const_1283:
               ...
               var_924 = new Profiler(this);
               attachComponent(var_924,[new IIDProfiler()]);
               var_751 = new ProfilerViewer();
               var_215.addChild(var_751);
               break;
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(var_1085)
         {
            ...
            var_1085.dispatchEvent(new ProgressEvent(ProgressEvent.PROGRESS,false,false,param1.bytesLoaded,param1.bytesTotal));
         }
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return var_326 - getNumberOfFilesPending();
      }
      ...
      private function debugFrameUpdateHandler(param1:Event) : void
      {
         ...
         var _loc3_:uint = _loc2_ - var_283;
         var_283 = _loc2_;
         _loc4_ = 0;
      ...
      private function profilerFrameUpdateHandler(param1:Event) : void
      {
         ...
         var msSinceLastUpdate:uint = msCurrentTime - var_283;
         var_283 = msCurrentTime;
         var_924.start();
         criticality = 0;
         while(criticality < CoreComponent.const_138)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     var_924.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(var_1085)
         {
            var_1085.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         try
         {
            node = config.child(const_1119)[0];
            if(node != null)
            {
               list = node.child(const_1114);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  var_281.push(url);
         ...
         try
         {
            node = config.child(const_1113)[0];
            if(node != null)
            {
               list = node.child(const_1116);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  var_282.push(url);
         ...
         try
         {
            node = config.child(const_1118)[0];
            if(node != null)
            {
               list = node.child(const_1117);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  var_284.push(url);
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1236);
         if(!_isDisposed)
         {
