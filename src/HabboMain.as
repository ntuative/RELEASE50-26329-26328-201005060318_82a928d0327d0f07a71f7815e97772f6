package
{
   ...
   public class HabboMain extends Sprite
   {
      ...
      private function updateLoadingBar(param1:DisplayObjectContainer, param2:Number) : void
      {
         var _loc11_:int = 0;
         var _loc12_:int = 0;
         var _loc7_:Sprite = param1.getChildByName(Habbo.const_4) as Sprite;
         var _loc8_:Sprite = _loc7_.getChildByName(Habbo.FILE_BAR_SPRITE) as Sprite;
         var _loc9_:Sprite = param1.getChildByName(Habbo.const_2) as Sprite;
         var _loc10_:Sprite = _loc9_.getChildByName(Habbo.const_9) as Sprite;
         var _loc13_:int = var_27.getNumberOfFilesPending() + var_27.getNumberOfFilesLoaded();
         var _loc14_:Number = 1 - const_24 + var_27.getNumberOfFilesLoaded() / _loc13_ * const_24;
         _loc8_.x = 1 + 1;
         _loc8_.y = 1 + 1;
      ...
      private function configLoadedHandler(param1:AssetLoaderEvent) : void
      {
         ...
         {
            Habbo.reportCrash("Failed to download external configuration document " + _loc2_.assetLoader.url + "!",Core.const_169,null);
         }
      }
      ...
      private function initializeCore() : void
      {
         ...
         catch(error:Error)
         {
            ...
            Core.crash("Failed to initialize the core: " + error.message,Core.const_913,error);
         }
      }
      ...
      protected function onAddedToStage(param1:Event = null) : void
      {
         ...
         {
            Habbo.trackLoginStep(Habbo.const_28);
            Habbo.reportCrash("Failed to prepare the core: " + error.message,Core.const_913,error);
            Core.dispose();
         }
      }
      ...
      private function setupCoreConfigFromLoader(param1:AssetLoaderStruct) : void
      {
         ...
         {
            Habbo.reportCrash("Download external configuration document is null!",Core.const_169,null);
         }
         if(var_27)
         {
         ...
         {
            Habbo.reportCrash("Core vanished while downloading config document!",Core.const_169,null);
         }
      }
